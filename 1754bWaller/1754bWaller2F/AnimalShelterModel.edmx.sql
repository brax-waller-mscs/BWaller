...
-- --------------------------------------------------
-- Date Created: 11/03/2020 18:14:24
-- Generated from EDMX file: C:\Users\braxd\OneDrive\Desktop\Fall 20\Visual Database\BWaller\1754bWaller\1754bWaller2F\AnimalShelterModel.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_DonationDonationType]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[DonationTypes] DROP CONSTRAINT [FK_DonationDonationType];
GO
...
-- Creating foreign key on [Donations_Id] in table 'DonationTypes'
ALTER TABLE [dbo].[DonationTypes]
ADD CONSTRAINT [FK_DonationDonationType]
    FOREIGN KEY ([Donations_Id])
    REFERENCES [dbo].[Donations]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO

-- Creating non-clustered index for FOREIGN KEY 'FK_DonationDonationType'
CREATE INDEX [IX_FK_DonationDonationType]
ON [dbo].[DonationTypes]
    ([Donations_Id]);
GO
